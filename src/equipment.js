...
P.equip=function(i,slot){const result=oldEquip.call(this,i,slot);if(!result)return result;if(i.name==='amulet of change'){this.player.gender=this.player.gender==='Female'?'Male':'Female';this.log('Your body changes.');this.consume(i);this.emit('appearance');}if(i.name==='helm of opposite alignment'){this.player.originalAlign||=this.player.align;this.player.align=this.player.align==='Lawful'?'Chaotic':'Lawful';i.buc=-1;this.log('Your allegiance shifts to an opposing deity.','warning');}return result;};
